...
install: build $(SOURCES_LIST) $(APT_KEY) install-repository.sh
	...
	install -D -m 0644 conf/nginx.conf $(DESTDIR)/etc/nginx/sites-available/digabi-repository-server
	install -D -m 0644 doc/index.html $(DESTDIR)/usr/share/digabi-repository/www/index.html
